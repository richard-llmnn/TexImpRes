...
class VariableResolver:
    def replace(self, file_content):
        ...
        for found_variable in semantics.get_all():
            if (
                file_content[found_variable.start() - 1] == "\\"
                and file_content[found_variable.start() - 2] != "\\"
            ):
                continue
            variables.append(
                {
        ...
        for variable in variables:
            ...
            matches = list(
                VariableSemantic.VariableSemantic(
                    file_content[start_from:], variable["key"]
                ).get_all()
            )
            matches.reverse()
            ...
            for found_variable in matches:
                if (
                    temp_file_content[found_variable.start() - 1] == "\\"
                    and temp_file_content[found_variable.start() - 2] != "\\"
                ):
                    skipped = True
                    continue
                skipped = False
                ...
                if (
                    temp_file_content[
                        found_variable.start() - 2 : found_variable.start()
                    ]
                    == r"\\"
                ):
                    file_content = (
                        file_content[: found_variable.start() + start_from - 1]
                        + file_content[found_variable.start() + start_from :]
                    )
            if not skipped:
                if file_content[variable["start"] - 2 : variable["start"]] == r"\\":
                    file_content = (
                        file_content[: variable["start"] - 1]
